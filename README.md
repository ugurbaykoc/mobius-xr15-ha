...
5. **`org.bluez.Error.InProgress` on every attempt, or commands that hang forever.** The adapter (or BlueZ's view of the device) is wedged — often after a connect that stalled and got cancelled. One command fixes it:
   ```bash
