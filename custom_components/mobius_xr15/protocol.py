"""Mobius C2 protocol packet builders.

Byte-for-byte the framing verified against a real XR15w G5 Pro and
cross-checked against the official app's own enums - see PROTOCOL.md.
No I/O here, so the wire format can be reasoned about on its own.
"""
...
def crc16(data: bytes) -> int:
    """CRC16 trailing every C2 request frame."""
    crc = 0xFFFF
    for byte in data:
...
def make_slot(time_min: int, flags: int, channel_values: dict[int, int] | None = None) -> bytes:
    """One 42-byte schedule slot: time + flags + 13 x (channel id, value)."""
    values = {channel: 0 for channel in CHANNELS_42}
    if channel_values:
        values.update(channel_values)
    channels = b"".join(
        bytes([channel]) + struct.pack("<H", max(0, min(1000, int(values[channel]))))
...
def build_write_sequence(slots: list[bytes], intensity: int) -> list[bytes]:
    """Install a 25-slot schedule, set intensity, resume playback."""
    padded = slots + [bytes(42)] * (SLOT_COUNT - len(slots))
    packets: list[bytes] = []
    msg_id = 1
    for start in range(0, SLOT_COUNT, BATCH_SIZE):
        packets.append(build_schedule_packet(start, padded[start : start + BATCH_SIZE], msg_id))
        msg_id += 1
...
def build_intensity_sequence(intensity: int) -> list[bytes]:
    """Retarget intensity without rewriting the schedule - 2 packets."""
    return [build_intensity_packet(intensity, 1), build_playback_packet(SCHED_RESUME, 2)]


def build_blank_schedule() -> list[bytes]:
    """All 25 slots empty - the schedule plays silence."""
